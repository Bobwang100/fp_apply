...
def no_tensor_pic_prepross(input_img, target_shortside_len):  # input shape is [:, :, 3]
    input_img = input_img - [103.939, 116.779, 123.68]
    old_height, old_width = np.shape(input_img)[0], np.shape(input_img)[1]
    if old_height < old_width:
        new_height, new_width = target_shortside_len, target_shortside_len * old_width // old_height
    else:
        new_height, new_width = target_shortside_len * old_height // old_width, target_shortside_len
    output_img = cv2.resize(input_img, (new_width, new_height))
    output_img_4dim = output_img[None, :, :, :]
    return output_img, output_img_4dim
