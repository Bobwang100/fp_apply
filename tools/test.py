...
from data.io.image_preprocess import no_tensor_pic_prepross
...
def parse_args():
  ...
  parser.add_argument('--weights', dest='weights',
                      # default='../data/pretrained_weights/resnet_v1_101.ckpt',
                      default='./output/pascal/res101_trained_weights/v1_pascal/pascal_90000model.ckpt',
  ...
  parser.add_argument('--test_dir', dest='test_dir',
                      help='test picture dir',
                      default='../data/test_dir', type=str)
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
      ...
      for i in tqdm(range(args.img_num)):
        ...
        img_test, img_test_4dim = no_tensor_pic_prepross(img_test, cfgs.SHORT_SIDE_LEN)
        ...
        _img_batch_fpn = help_utils.draw_box_cv(img_test,
        ...
        cv2.imwrite(cfgs.TEST_SAVE_PATH +
              '/{}_fpn.jpg'.format(str(test_filenames[i])), _img_batch_fpn)
        # cv2.imwrite(cfgs.TEST_SAVE_PATH +
